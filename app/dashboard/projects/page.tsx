...
const TYPE_META: Record<ProjectType, { label: string; icon: any; color: string; bg: string; desc: string }> = {
  code: { label: 'Code', icon: HiCode, color: '#818cf8', bg: 'rgba(129,140,248,0.12)', desc: 'Full IDE with real Linux environment and AI agents' },
  design: { label: 'Design', icon: BsBrush, color: '#f472b6', bg: 'rgba(244,114,182,0.12)', desc: 'Figma import + AI UI generation + LibreOffice Draw' },
  document: { label: 'Document', icon: BsFileEarmarkRichtext, color: '#34d399', bg: 'rgba(52,211,153,0.12)', desc: 'AI-powered docs with LibreOffice conversion' },
};
...
export default function ProjectsPage() {
  ...
  const createProject = async () => {
    ...
    // Init the real workspace directory on the server
    try {
      await fetch(`/api/workspace/${proj.id}/init`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
    } catch { /* non-fatal */ }
    ...
    // Navigate directly into the new project workspace
  ...
  const importGithub = async () => {
    ...
    try {
      ...
      // Init and clone into the real workspace
      try {
        await fetch(`/api/workspace/${proj.id}/init`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
        await fetch(`/api/workspace/${proj.id}/terminal`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ command: `git clone ${githubUrl.trim()} . 2>&1 | tail -5` }),
        });
      } catch { /* non-fatal */ }
  ...
  const deleteProject = (id: string) => {
    ...
    setMenuOpen(null);
    // Also clean up server-side directory
    fetch(`/api/workspace/${id}/files`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ path: '.' }) }).catch(() => {});
  };
    ...
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>AI workspaces with real Linux environment + LibreOffice</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="glass rounded-2xl p-14 text-center">
          ...
          <p className="text-sm mb-5" style={{ color: 'var(--text-secondary)' }}>Create a code, design, or document project to get started</p>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(p => {
            ...
            return (
              <div key={p.id} className="glass glass-hover rounded-2xl p-5 group relative transition-all duration-200 cursor-pointer"
                ...
                {/* Menu */}
                <div className="absolute top-3 right-3" data-no-nav>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(12px)' }}>
          <div className="glass rounded-2xl p-7 w-full max-w-md" style={{ boxShadow: '0 0 60px rgba(99,102,241,0.15)' }}>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={createProject} disabled={creating || !newName.trim()} className="btn-primary flex-1 gap-2 flex items-center justify-center" style={{ opacity: creating ? 0.7 : 1 }}>
                {creating ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Creating...</> : <><HiLightningBolt className="w-4 h-4" /> Create & Open</>}
              </button>
            </div>
          </div>
        </div>
      ...
      {showGithub && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(12px)' }}>
          <div className="glass rounded-2xl p-7 w-full max-w-md" style={{ boxShadow: '0 0 60px rgba(99,102,241,0.15)' }}>
            ...
            <p className="text-sm mb-4" style={{ color: 'var(--text-secondary)' }}>Enter a public GitHub repository URL. The repo will be cloned into your real Linux workspace.</p>
            <div>
            ...
            <div className="glass rounded-xl p-3 mt-4 text-xs" style={{ color: 'var(--text-muted)' }}>
              <strong style={{ color: 'var(--text-secondary)' }}>Note:</strong> Only public repositories are supported. The code runs in a real /tmp workspace.
            </div>
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={importGithub} disabled={importing} className="btn-primary flex-1 gap-2 flex items-center justify-center" style={{ opacity: importing ? 0.6 : 1 }}>
                {importing ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Cloning...</> : <><BsGithub className="w-4 h-4" /> Clone & Open</>}
              </button>
            </div>
          </div>
        </div>
      ...
      {showFigma && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(12px)' }}>
          <div className="glass rounded-2xl p-7 w-full max-w-md" style={{ boxShadow: '0 0 60px rgba(244,114,182,0.15)' }}>
            ...
            <p className="text-sm mb-4" style={{ color: 'var(--text-secondary)' }}>Paste a Figma file or frame URL. The AI coding agent will help convert it to working component code.</p>
            <div>
